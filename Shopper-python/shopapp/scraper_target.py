...
async def target_search_products_async(
    ...
    """
    Async Target.com search -> list[Product].
    """
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Target uses data-test attribute for product cards
            cards = await page.query_selector_all('[data-test="@web/site-top-of-funnel/ProductCardWrapper"]')
            if len(cards) == 0:
                # Try alternative selector
                cards = await page.query_selector_all('section[data-test*="product"]')
            
            print(f"Found {len(cards)} Target product cards")
        ...
        for idx, card in enumerate(cards):
            ...
            try:
                ...
                title = None
                title_link = await card.query_selector('a[data-test="product-title"]')
                if not title_link:
                ...
                # Rating - look for pattern like "4.5" or "4.5 out of 5"
                rating = None
